...
Item {
    ...
    component SoundBars: Row {
        ...
        Repeater {
            model: barCount
            Rectangle {
                width: soundBars.barWidth
                radius: width / 2
                color: soundBars.color
                // 每根 bar 基于相位做不同幅度，营造均衡器效果
                height: soundBars.playing ? Qt.binding(() => {
                    var phase = (index / soundBars.barCount) * Math.PI * 2;
                    var t = Date.now() / 180;
                    var s = (Math.sin(t + phase) + 1) / 2; // 0..1
                    return soundBars.minHeight + s * (soundBars.maxHeight - soundBars.minHeight);
                }) : soundBars.minHeight
                y: (parent.height - height) / 2
                opacity: soundBars.playing ? 0.9 : 0.4
                Behavior on height {
                    enabled: !soundBars.playing
